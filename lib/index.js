...
function displayFoods(foodItemsArray) {
  ...
  foodItemsArray.forEach(function(element) {
    ...
    $('#food-grid').append(`
			...
			<input type="checkbox" name=${formatName(name,true)} id="checkbox-${id}" class="checkbox-inactive" onclick="toggleCheckbox(${id})">
...
function displayRecipe(image,title,id){
	...
	$('.recipe-grid').append(`
		<div id=${id}>
			...
			<a onclick="location.href='https://www.yummly.com/recipe/${id}'"><img width = 80px  src=${image}></a>
		</div>
		`)
}
