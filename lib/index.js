...
function displayDiaryIndex (mealsArray) {
	...
	var breakCal = buildTable(breakfastTable, mealsArray.rows[0]) || 0;
	var dinnerCal = buildTable(dinnerTable, mealsArray.rows[1]) || 0;
	var snackCal = buildTable(snackTable, mealsArray.rows[2]) || 0;
  var lunchCal = buildTable(lunchTable, mealsArray.rows[3]) || 0;
...
function getRecipesApi(search){
	debugger;
	fetch(`http://api.yummly.com/v1/api/recipes?_app_id=${yummly.apiId}&_app_key=${yummly.apiKey}&${search}`)
	.then((response)=>{
		debugger;
	})
}
